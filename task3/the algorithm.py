...
class GA:
    ...
    def crossover(self, DNA_list):
        ...
        newDNAs = []
        #crossover
        for k in range(self.population_size - len(newDNAs)):

            '''choose partners without replacement so that same dna doesnot goes to both the partners'''
            partner1, partner2 = np.random.choice(DNA_list, size=2, replace=False)
            offspring1, offspring2 = np.empty_like(partner1),np.empty_like(partner2)

            #Using Uniform Crossover
            for i in range(len(partner1)):
                if np.random.rand() < 0.5:
                    offspring1[i] = partner1[i]
                    offspring2[i] = partner2[i]
                else:
                    offspring1[i] = partner2[i]
                    offspring2[i] = partner1[i]
                newDNAs.extend([offspring1, offspring2])         
    ...
    def mutation(self, DNA):
        ...
        for gene in DNA:
            if np.random.rand() < self.mutation_rate:
                gene += np.random.randn(*gene.shape) * 0.1
    ...
    def next_generation(self):
        ...
        #first sort in ascending order and then take last two indexes
        index_good_fitness = np.argsort(self.current_fitness)[-2:] #index of parents selected for crossover.
        #fill the list.
        ...
        for index in index_good_fitness:
            w1 = self.current_generation[0][index] # input layer weight
            dna_in_w = w1.reshape(w1.shape[1], -1)

            b1 = self.current_generation[1][index] # input layer biases
            dna_b1 = np.append(dna_in_w, b1)

            w2 = self.current_generation[2][index] # output layer weight
            dna_whid = w2.reshape(w2.shape[1], -1)
            dna_w2 = np.append(dna_b1, dna_whid)

            wh = self.current_generation[3][index] # hidden layer weight
        ...
        for newdna in new_DNA_list:
            ...
            new_fitness_list.append(self.run_environment(newdna[0], newdna[1], newdna[3], newdna[2]))
    ...
    def evolve(self):
        ...
        for gen in range(self.number_of_generation):
            ...
            if max_fitness > self.best_fitness:
                self.best_gen = new_generation[np.argmax(new_fitness_list)]
...
def trainer():
    ...
    ga_optimiser = GA(init_weight_list, init_fitness_list, num_of_generation, pop_size, learner)
    #call evolve function to obtain optimized weights.
    optimised_weights, optimised_fitness = ga_optimiser.evolve()
